class Canvas {
  ...
  findByTag(tag) {
    ...
    if (this.gameObjectsByTag.has(tag))
      results = this.gameObjectsByTag.get(tag);
  ...
  appendObject(obj) {
    try {
      ...
      if (this.gameObjectsById.has(obj.ID))
        throw Canvas.errors.get("GO_DUP_ID");
  ...
  drawBackground() {
    this.drawRect(
      Vector.substract(Vector.zero(), this.offset.copy()),
